...
import { buildBlocklistConfig } from "../src/blocklist";
import { CommunityStoreDO } from "../src/durable-objects/community-store";
...
test("CommunityStoreDO starts with no globally blocked emojis", async () => {
  const sql = createFakeSql();
  const ctx = { storage: { sql } } as unknown as DurableObjectState;
  const env = { BOT_USER_ID: "bot-1" } as never;

  const store = new CommunityStoreDO(ctx, env);
  const response = await store.fetch(new Request("https://community-store/config"));
...
test("CommunityStoreDO preserves stored bot user id across reconstruction", async () => {
  const ctx = {
    storage: { sql: createFakeSql() },
  } as unknown as DurableObjectState;
  const store = new CommunityStoreDO(ctx, { BOT_USER_ID: "env-bot-id" } as never);

  const writeResponse = await store.fetch(
    new Request("https://community-store/app-config", {
      method: "POST",
  ...
  const rehydratedStore = new CommunityStoreDO(ctx, { BOT_USER_ID: "fresh-env-bot-id" } as never);
  const response = await rehydratedStore.fetch(new Request("https://community-store/config"));
  const config = (await response.json()) as { botUserId: string };

  assert.equal(config.botUserId, "stored-bot-id");
});

test("CommunityStoreDO maps invalid input to 400 and storage faults to 500", async () => {
  const ctx = {
    storage: { sql: createFakeSql() },
  } as unknown as DurableObjectState;
  const env = { BOT_USER_ID: "bot-1" } as never;
  ...
  const invalidJsonResponse = await store.fetch(
    new Request("https://community-store/guild-emoji", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "{",
    }),
  ...
  const failingStore = new CommunityStoreDO(
    {
...
test("CommunityStoreDO rejects empty app-config keys with 400", async () => {
  const ctx = {
    storage: { sql: createFakeSql() },
  } as unknown as DurableObjectState;
  const env = { BOT_USER_ID: "bot-1" } as never;
...
test("CommunityStoreDO returns 500 when config reads hit storage faults", async () => {
  const ctx = {
  ...
  console.error = () => {};
  try {
    response = await store.fetch(new Request("https://community-store/config"));
  } finally {
    console.error = originalConsoleError;
  }
...
test("guild-scoped emoji add and remove", async () => {
  ...
  const configResponse = await store.fetch(new Request("https://community-store/config"));
  ...
  const configResponse2 = await store.fetch(new Request("https://community-store/config"));
...
test("CommunityStoreDO stores and clears the guild moderation log channel", async () => {
  const sql = createFakeSql();
  const ctx = { storage: { sql } } as unknown as DurableObjectState;
  const env = { BOT_USER_ID: "bot-1" } as never;
  const store = new CommunityStoreDO(ctx, env);
  ...
  const saveResponse = await store.fetch(
    new Request("https://community-store/guild-notification-channel", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        guildId: "guild-1",
  ...
  const readResponse = await store.fetch(
    new Request("https://community-store/guild-notification-channel?guildId=guild-1"),
  );
...
test("CommunityStoreDO upserts and lists active timed roles by guild", async () => {
  const alarms: number[] = [];
  const ctx = {
    storage: {
      sql: createFakeSql(),
  ...
  const store = new CommunityStoreDO(ctx, {
    BOT_USER_ID: "bot-1",
    DISCORD_BOT_TOKEN: "token",
  } as never);

  const response = await store.fetch(
    new Request("https://community-store/timed-role", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        guildId: "guild-1",
  ...
  const listResponse = await store.fetch(
    new Request("https://community-store/timed-roles?guildId=guild-1"),
  );
...
test("CommunityStoreDO stores new member timed role configuration", async () => {
  const ctx = {
    storage: {
      sql: createFakeSql(),
      setAlarm() {
  ...
  const saveResponse = await store.fetch(
    new Request("https://community-store/timed-role/new-member-config", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        guildId: "guild-1",
  ...
  const readResponse = await store.fetch(
    new Request("https://community-store/timed-role/new-member-config?guildId=guild-1"),
  );
...
test("CommunityStoreDO replaces timed role expiry when upserting the same assignment", async () => {
...
test("CommunityStoreDO removes timed roles via route", async () => {
  const ctx = {
    storage: {
      sql: createFakeSql(),
      setAlarm() {
  ...
  const deleteResponse = await store.fetch(
    new Request("https://community-store/timed-role/remove", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        guildId: "guild-1",
...
test("CommunityStoreDO clears the timed role alarm when the last assignment is removed", async () => {
  const alarms: number[] = [];
...
test("CommunityStoreDO stores ticket panels and ticket instances through HTTP endpoints", async () => {
  const sql = createFakeSql();
  const ctx = { storage: { sql } } as unknown as DurableObjectState;
  const store = new CommunityStoreDO(ctx, {
    BOT_USER_ID: "bot-1",
  ...
  const savePanel = await store.fetch(
    new Request("https://community-store/ticket-panel", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  const readPanel = await store.fetch(
    new Request("https://community-store/ticket-panel?guildId=guild-1"),
  );
  assert.deepEqual(await readPanel.json(), panel);

  const createTicket = await store.fetch(
    new Request("https://community-store/ticket-instance", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        guildId: "guild-1",
  ...
  const readOpen = await store.fetch(
    new Request(
      "https://community-store/ticket-instance/open?guildId=guild-1&channelId=ticket-channel-1",
    ),
  );
  ...
  const closeTicket = await store.fetch(
    new Request("https://community-store/ticket-instance/close", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        guildId: "guild-1",
  ...
  const reserveTicketOne = await store.fetch(
    new Request("https://community-store/ticket-number/next", {
      method: "POST",
      headers: { "content-type": "application/json" },
...
test("CommunityStoreDO rejects invalid ticket payloads with 400", async () => {
  const sql = createFakeSql();
  const ctx = { storage: { sql } } as unknown as DurableObjectState;
  const store = new CommunityStoreDO(ctx, {
    BOT_USER_ID: "bot-1",
...
test("CommunityStoreDO surfaces a failed ticket close update", async () => {
...
test("CommunityStoreDO alarm only removes timed roles after Discord role removal succeeds", async () => {
  ...
  try {
    ...
    await (store as CommunityStoreDO & { alarm(): Promise<void> }).alarm();
